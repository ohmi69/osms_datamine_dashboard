...
export function buildSkillRangeVisual(skill) {
  ...
  titleWrap.appendChild(el('div', { className: 'skill-range-eyebrow', textContent: 'Range & targeting' }));
  ...
  const showsMetrics = usesWeaponAction || usesStoredRectangle;
  const stage = el('div', {
    className: `hitviz-stage skill-range-stage${showsMetrics ? ' skill-range-stage--with-metrics' : ''}`,
  });
