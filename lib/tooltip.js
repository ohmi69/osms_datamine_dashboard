...
export function showPortalTooltip(e, portalData) {
  ...
  tip.appendChild(header);
  console.log({portal})
  // Intra-map: just say it teleports within the same map
  if (portal.intra_map) {
    tip.appendChild(el('div', { style: { margin: '6px 0 2px 0', fontSize: '13px' }, textContent: 'Teleports within the same map.' }));
  } else {
