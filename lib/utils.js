...
// Keeps dense filter controls out of the way on phones without changing the
// desktop layout. Callers append their controls to `content`.
export function makeResponsiveFilterPanel(label = 'Filters') {
  const panel = el('section', { className: 'responsive-filter-panel' });
  const contentId = `filter-panel-${Math.random().toString(36).slice(2, 9)}`;
  const button = el('button', {
    className: 'responsive-filter-panel__toggle',
    textContent: label,
    type: 'button',
    'aria-controls': contentId,
  });
  const content = el('div', { className: 'responsive-filter-panel__content', id: contentId });
  const startsCollapsed = window.matchMedia?.('(max-width: 640px)').matches ?? false;
