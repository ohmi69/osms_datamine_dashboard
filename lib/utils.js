...
// A sticky <thead> can only pin to the scroll container it sits in, and
// .table-scroll is one whenever overflow-x is active - so the header would ride
// along with the wrapper instead of the viewport. Drop the wrapper's scroll
// container on the widths where the table fits anyway, and keep it (no sticky
// header) on the narrow ones that genuinely need to scroll sideways.
export function enableStickyTableHead(wrapper, table) {
  const apply = () => {
    if (!wrapper.isConnected || !wrapper.clientWidth) return;
    // Measures the table, not the wrapper, so the answer doesn't depend on which
    // mode we're currently in - no flip-flopping between the two.
    wrapper.classList.toggle('table-scroll--sticky-head', table.scrollWidth <= wrapper.clientWidth + 1);
  };
  apply();
  // Fires on insertion as well as on resize, which covers tables built inside a
  // collapsible that hasn't been opened yet.
  new ResizeObserver(apply).observe(wrapper);
}

// Scrolls a row to just below whatever is pinned at the top of the viewport
// (site header, sticky toolbar, and the table's own pinned column header).
function scrollRowToTop(row) {
  ...
  const head = row.closest('.table-scroll--sticky-head')?.querySelector('thead');
  const offset = Math.max(headerHeight + toolbarHeight + (head ? head.offsetHeight : 0), 64);
