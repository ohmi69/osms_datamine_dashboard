...
const BASE_DAMAGE_STEPS = [
  {
    ...
    notes: [
      ...
      'Lucky Seven is the only skill in the client that gets a special weapon multiplier. Drain is not one of them - it uses the ordinary Claw throw value of 2.5. The two skills are otherwise identical in the skill data (same weapon, same properties, neither has its own animation), which is almost certainly why testing suggested they shared a multiplier',
...
const BASE_DAMAGE_VARS = [
  ...
  { name: 'MasteryMult',   desc: '(0.1 + MasteryLevel / 10) × 0.8. Exception: Lucky Seven ignores mastery entirely and uses a flat 0.5, the same as a MasteryLevel of 5.25. No other skill, Drain included, gets that exception' },
